...
import BillboardMessage from "./index";
...
function BillboardStory(): JSX.Element {
  ...
  const showBillboard = (): void => {
    billboard.show(
      <BillboardMessage text="Some React component to show on billboard. Please interact with NEUMA before continue." />,
    );
    setTimeout(() => {
      billboard.close();
    }, 3000);
  ...
  return (
    <React.Fragment>
      ...
      <Button color="primary" onClick={showBillboard}>
        Show Neuma billboard
      </Button>
    </React.Fragment>
  );
}
