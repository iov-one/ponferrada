...
export const travelTo = async (
  ...
  const dom = createDom(requests, persona, hasPersona);
...
export const travelToWelcome = async (hasPersona: boolean): Promise<React.Component> => {
  return travelTo(WELCOME_ROUTE, [], null, hasPersona);
};
